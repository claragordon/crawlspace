...
class WebScraper:
  def __init__(self, max_workers: int, max_depth: int, max_outlinks: int, rate_capacity: int, rate_per_second: float, request_timeout: int):
  ...
  def _parse(self, url: str, html: str) -> UrlResult:
    ...
    outlinks = [urljoin(url, link.get("href")) for link in soup.find_all("a", href=True)]
    ...
    while True:
      ...
      try:
        ...
        if result:
          ...
          for outlink in result.outlinks[:min(self.max_outlinks, len(result.outlinks))]:
...
def parse_args(argv: List[str]) -> argparse.Namespace:
  ...
  p.add_argument("--max-workers", type=int, default=5, help="Number of worker threads")
  p.add_argument("--max-depth", type=int, default=2, help="Maximum crawl depth")
  p.add_argument("--max-outlinks", type=int, default=5, help="Maximum outlinks followed per page")
  p.add_argument("--rate-capacity", type=float, default=5.0, help="Token bucket capacity")
  p.add_argument("--rate-per-second", type=float, default=1.0, help="Token bucket refill rate")
